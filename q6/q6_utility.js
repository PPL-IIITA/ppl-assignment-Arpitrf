...
try{
	rl.question("Enter the t days: ", function(t) {
		for(var i = 0 ; i < t ; i++) {
			func(i);
		}
		rl.close();
...
function func(t_days) {
	...
	var k = 0;
		for(var i  = 0 ; i < t_days ; i++) {
			fileWrite_breakup(arr_couple[i], k++);
...
function fileWrite_breakup(arr_couple, i) {
	var y = "\n\n\n"+i + "th DAY:\n\n\n"
	fs.appendFile("./list.txt",y);
	var x = "\n\nBREAKUPS:\n\n"
	fs.appendFile("./list.txt",x);
